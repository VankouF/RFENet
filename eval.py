...
import numpy as np
from datasets import GSD, PMD, Trans10k
...
parser.add_argument('--dataset_cls', type=str, default='Trans10k')
...
def setup_loader():
    ...
    if args.dataset == 'Trans10k' or args.dataset == 'PMD' or args.dataset == 'GSD':
        val_joint_transform_list = [joint_transforms.Resize(args.resize_scale)]
        val_input_transform = transforms.Compose([transforms.ToTensor()])
        target_transform = extended_transforms.MaskToTensor()

    if args.dataset == 'PMD':
        args.dataset_cls = PMD
        test_set = args.dataset_cls.PMDDateset(args.mode, args.split,
                                               joint_transform_list=val_joint_transform_list,
                                               transform=val_input_transform,
                                               target_transform=target_transform)
    elif args.dataset == 'Trans10k':
        args.dataset_cls = Trans10k
        test_set = args.dataset_cls.Trans10kDataset(args.mode, args.split,
                                                    joint_transform_list=val_joint_transform_list,
                                                    transform=val_input_transform,
                                                    target_transform=target_transform)
    elif args.dataset == 'GSD':
        args.dataset_cls = GSD
        test_set = args.dataset_cls.GSDDateset(args.mode, args.split,
                                               joint_transform_list=val_joint_transform_list,
                                               transform=val_input_transform,
                                               target_transform=target_transform)
    else:
        ...
        if self.metrics:
            if not self.with_mae_ber:
                ...
                evaluate_eval_for_inference_with_mae_ber(self.hist, self.total_mae,
                                                         ...
                                                         dataset=args.dataset_cls, with_f1=args.with_f1, beta=self.beta)
