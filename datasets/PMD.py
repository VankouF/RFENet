...
def make_dataset(quality, mode):
    ...
    logging.info(f'PMD has a total of {len(all_items)} images in {mode} phase')

    logging.info(f'PMD-{mode}: {len(all_items)} images')

    return all_items


class PMDDateset(data.Dataset):
