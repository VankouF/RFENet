...
class MCLoss(nn.Module):
    ...
    def __init__(self, classes, ignore_index=255, mode='train', edge_weight=0.25,
                 seg_weight=1.0, middle_weight=0.01, ohem=False, dice=False):
    ...
    def forward(self, inputs, gts):
        seg_final_in, seg_ins, edge_ins = inputs
        mask, edge_mask = gts
        ...
        for seg_in in seg_ins:
            seg_loss = seg_loss + self.seg_loss(seg_in, mask.clone())

        seg_loss = self.middle_weight * seg_loss + self.seg_weight * self.seg_loss(seg_final_in, mask.clone())

        losses['seg_loss'] = seg_loss

        if not self.dice_loss:
            for edge_in in edge_ins:
                edge_loss = edge_loss + self.bce2d(edge_in, edge_mask.clone())
            losses['edge_loss'] = self.edge_weight * edge_loss
        else:  # dice loss branch
            device = edge_ins[0].device
            edge_mask = edge_mask.clone()
            edge_mask.to(device)
            valid = torch.ones_like(edge_mask)
            for edge_in in edge_ins:
                edge_in = F.sigmoid(edge_in)
                edge_loss = edge_loss + self.edge_loss(edge_in, edge_mask, valid)
            losses['edge_loss'] = self.edge_weight * edge_loss
...
def get_loss(args):
    criterion = MCLoss(classes=args.dataset_cls.num_classes,
