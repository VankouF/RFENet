...
def prep_experiment(args, parser):
    ...
    args.exp_path = os.path.join(ckpt_path, 'train')
    args.tb_exp_path = os.path.join(tb_path, 'train')
...
def evaluate_eval_for_inference(hist, dataset=None, with_f1=False, beta=1):
    ...
    mean_iu_without_bkg = np.nanmean(iu[1:])
    if with_f1:
        mean_f1 = np.nanmean(f1s)

    logging.info('mean iou {}'.format(mean_iu_without_bkg))
    if with_f1:
...
def evaluate_eval_for_inference_with_mae_ber(hist, total_maes, total_bers, total_bers_count,
                                             dataset=None, with_f1=False, beta=1):
    all_acc = np.diag(hist).sum() / hist.sum()
    acc_cls = np.diag(hist) / hist.sum(axis=1)
    macc = np.nanmean(acc_cls)
    ...
    if with_f1:
        f1s = print_evaluate_results_acc_iou_mae_ber_f1(hist, iou, acc_cls, ber, beta=beta, dataset=dataset)
        mean_f1 = np.nanmean(f1s)
        mean_f1_without_bkg = np.nanmean(f1s[1:])
        logging.info(f'mean f1 {mean_f1_without_bkg}')
    ...
    miou_without_bkg = np.nanmean(iou[1:])
    mBer_without_bkg = np.nanmean(ber[1:])

    '''if with_f1:
        f1s = print_evaluate_results_f1(hist, iou, beta=beta, dataset=dataset)
        mean_f1 = np.nanmean(f1s)
        logging.info(f'mean f1 {mean_f1}')'''

    logging.info(f'mean iou: {miou_without_bkg}')
    logging.info(f'macc: {macc}')
    logging.info(f'all acc: {all_acc}')
    logging.info(f'mae: {mae}')
    logging.info(f'mBer: {mBer_without_bkg}')

    return all_acc, acc_cls, macc, miou, mae, mBer


def evaluate_eval(args, net, optimizer, val_loss, hist, mae, bers, bers_count,
                  dump_images, writer, epoch=0, dataset=None):
    """
    Modified IOU mechanism for on-the-fly IOU calculations ( prevents memory overflow for
    large dataset) Only applies to eval/eval.py
    """
    ...
    if args.with_f1:
        f1s = print_evaluate_results_acc_iou_mae_ber_f1(hist, iu, acc_c, Ber, beta=args.beta, dataset=dataset)
        mean_f1 = np.nanmean(f1s)
        mean_f1_without_bkg = np.nanmean(f1s[1:])

        logging.info(f'mean f1 {mean_f1_without_bkg}')
    else:
        print_evaluate_results_acc_iou_mae_ber(hist, iu, acc_c, Ber, dataset)
    ...
    logging.info('mean_acc {}'.format(acc_cls))
    logging.info('mean_iou {}'.format(mean_iu_without_bkg))

    logging.info('mae {}'.format(mae))
    logging.info('mBer {}'.format(mBer_without_bkg))
    ...
    if args.with_f1:
        torch.save({
            ...
            'epoch': epoch,
            'mean_iu': mean_iu,
            'mean_iu_without_bkg': mean_iu_without_bkg,
            'mae': mae,
            'mBer': mBer,
            'mBer_without_bkg': mBer_without_bkg,
            'mean_f1_without_bkg': mean_f1_without_bkg,
    ...
    if mean_iu_without_bkg > args.best_record['mean_iu_without_bkg'] or \
            (args.with_f1 and abs(
                mean_iu_without_bkg - args.best_record['mean_iu_without_bkg']) < 0.0005 and mean_f1_without_bkg >
             args.best_record['mean_f1_without_bkg']) or \
            (abs(mean_iu_without_bkg - args.best_record['mean_iu_without_bkg']) < 0.0005 and \
             mBer_without_bkg < args.best_record['mBer_without_bkg']):
        ...
        args.best_record['mean_iu_without_bkg'] = mean_iu_without_bkg
        args.best_record['mBer'] = mBer
        args.best_record['mBer_without_bkg'] = mBer_without_bkg
        args.best_record['mae'] = mae
        if args.with_f1:
            args.best_record['mean_f1_without_bkg'] = mean_f1_without_bkg
        ...
        shutil.copyfile(last_snapshot, os.path.join(args.log_root, "best.pth"))
    ...
    if args.with_f1:
        fmt_str = '[epoch %d], [val loss %.5f], [acc %.5f], [acc_cls %.5f], ' + \
                  ...
                  '[f1 %.5f]' + \
                  '[fwavacc %.5f]'
        logging.info(fmt_str % (epoch, val_loss.avg, acc, acc_cls,
                                mean_iu_without_bkg,
                                mae, mBer_without_bkg, mean_f1_without_bkg,
                                fwavacc))
        fmt_str = 'best record: [val loss %.5f], [acc %.5f], [acc_cls %.5f], ' + \
                  ...
                  '[f1 %.5f]' + \
                  '[fwavacc %.5f], [epoch %d]'
        logging.info(fmt_str % (args.best_record['val_loss'], args.best_record['acc'], args.best_record['acc_cls'],
                                args.best_record['mean_iu_without_bkg'],
                                args.best_record['mae'], args.best_record['mBer_without_bkg'],
                                args.best_record['mean_f1_without_bkg'],
        ...
        fmt_str = '[epoch %d], [val loss %.5f], [acc %.5f], [acc_cls %.5f], ' + \
                  ...
                  '[mae %.5f], [mBer %.5f], ' + \
                  '[fwavacc %.5f]'
        logging.info(fmt_str % (epoch, val_loss.avg, acc, acc_cls, mean_iu_without_bkg,
                                mae, mBer_without_bkg,
                                fwavacc))
        fmt_str = 'best record: [val loss %.5f], [acc %.5f], [acc_cls %.5f], ' + \
                  '[mean_iu %.5f], ' + \
                  '[mae %.5f], [mBer %.5f], ' + \
    ...
    writer.add_scalar('training/mean_iou', mean_iu_without_bkg, epoch)
    writer.add_scalar('training/mBer', mBer_without_bkg, epoch)
...
def print_evaluate_results_acc_iou_mae_ber_f1(hist, iou, acc, Ber, beta=1, dataset=None):
    try:
        id2cat = dataset.id2cat
    except:
        id2cat = {i: i for i in range(dataset.num_classes)}
    ...
    logging.info('label_id     label     iou    acc    Ber    Precision Recall F_beta')
    for idx, i in enumerate(iou):
        idx_string = "{:2d}".format(idx)
        class_name = "{:>13}".format(id2cat[idx] if idx in id2cat else '')
        iou_string = "{:5.5f}".format(i * 100)
        ...
        pre = true_positive[idx] / (true_positive[idx] + false_positive[idx])
        rec = true_positive[idx] / (true_positive[idx] + false_negative[idx])
